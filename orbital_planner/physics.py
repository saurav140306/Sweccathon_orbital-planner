"""Earth + optional third-body gravity with RK4 integration and impulsive burns."""
...
def _state_vector(position: tuple[float, float], velocity: tuple[float, float]) -> np.ndarray:
    return np.array([position[0], position[1], velocity[0], velocity[1]], dtype=float)


def _unpack(state: np.ndarray) -> tuple[tuple[float, float], tuple[float, float]]:
...
def total_acceleration(
    ...
    *,
    ...
    """Sum of Earth central gravity and moon third-body pull (if applicable)."""
...
def acceleration(position: np.ndarray, mu: float = MU_EARTH_KM3_S2) -> np.ndarray:
    """Earth-only acceleration (legacy helper)."""
    return earth_acceleration(position, mu)


def gravity_breakdown_at(
    chaser_position: tuple[float, float],
    t_s: float,
    target: TargetSpec | None,
) -> dict[str, float]:
    """Magnitudes for the calculations panel (km/s²)."""
    pos = np.array(chaser_position, dtype=float)
    ...
    moon_pos = (
        target_position_at(target, t_s)
        if target is not None and target_body_mu(target) > 0.0
        else (0.0, 0.0)
    )
    rel_dist = float(np.linalg.norm(np.array(moon_pos) - pos)) if target else 0.0
    return {
...
def _derivatives(
    ...
    return np.array([velocity[0], velocity[1], accel[0], accel[1]], dtype=float)
...
def apply_burn(velocity: tuple[float, float], dv: tuple[float, float]) -> tuple[float, float]:
    return (velocity[0] + dv[0], velocity[1] + dv[1])
...
def simulate_trajectory(
    *,
    ...
    """
    Integrate the orbit under Earth + moon gravity, applying impulsive burns.

    The moon (when target.kind == \"moon\") moves on a prescribed Kepler orbit;
    its mass perturbs the chaser but the chaser does not affect the moon.

    Returns:
        trajectory samples, crashed flag, propagation time, unused placeholder.
    """
    burns_sorted = sorted(burns, key=lambda b: b.time_s)
    burn_index = 0

    state = _state_vector(position, velocity)
    trajectory: list[TrajectoryPoint] = [
        TrajectoryPoint(t_s=0.0, position=position, velocity=velocity)
    ]
    next_sample_t = sample_every_s if sample_every_s is not None else time_limit_s + 1.0

    t = 0.0
    crashed = np.linalg.norm(state[:2]) < earth_radius_km
...
def min_distance_to_target(
    ...
    """Static target — legacy helper."""
    target = np.array(target_position, dtype=float)
    best_dist = float("inf")
    best_t = 0.0
    for point in trajectory:
        dist = float(np.linalg.norm(np.array(point.position) - target))
        if dist < best_dist:
            best_dist = dist
            best_t = point.t_s
...
def min_distance_to_moving_target(
    ...
    """Return (miss_km, time_of_closest_approach_s) vs moving target."""
    best_dist = float("inf")
    best_t = 0.0
    for point in trajectory:
        tgt = np.array(target_position_at(target, point.t_s), dtype=float)
        dist = float(np.linalg.norm(np.array(point.position) - tgt))
        if dist < best_dist:
            best_dist = dist
            best_t = point.t_s
    return best_dist, best_t
